...
def getCoinData(coin, currency):
    ...
    url=main_api+coin
    url=url+'/?convert='+currency.upper()
    ...
    if 'error' in json_data_end: #This will check if the entered name exists, if not, check if the symbol exists
        #Two links because loading the whole thing will be slow, top 200 at first because that may be fast enough. Second link loads whole json after 198, allows 2 coin overlap. Converted to chosen currency.
        url='https://api.coinmarketcap.com/v1/ticker/?start=0&limit=200&convert='+currency #url with json data on all coins on site, limit=0 to get all
        json_data_end=jsonSearch(url, coin)
        if json_data_end==None:
            url='https://api.coinmarketcap.com/v1/ticker/?start=198&limit=0&convert='+currency
            json_data_end=jsonSearch(url, coin)           
