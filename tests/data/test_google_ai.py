...
from data.google_ai import generate_text, GoogleAIError, generate_content
...
def test_generate_text_litellm_error(mock_completion):
    ...
    with pytest.raises(GoogleAIError, match="Error when calling litellm: LiteLLM Error"):
        generate_text("test")
